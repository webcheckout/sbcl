...
(defun old-eval-progn-body (progn-body lexenv)
  ...
  (do* ((i progn-body rest-i)
	...
	(old-eval-in-lexenv (first i) lexenv)
	(return (old-eval-in-lexenv (first i) lexenv)))))

(defun old-eval-locally (exp lexenv &key vars)
    ...
    (let ((lexenv
		  ...
		  (sb!c::*undefined-warnings* nil))
             ...
             (sb!c::process-decls decls
                                  ...
                                  nil
                                  :lexenv lexenv))))
      (old-eval-progn-body body lexenv))))

;;; Pick off a few easy cases, and the various top level EVAL-WHEN
;;; magical cases, and call %OLD-EVAL for the rest.
(defun old-eval-in-lexenv (original-exp lexenv)
		...
		(do ((name (cdr exp) (cddr name)))
		    ((null name)
			 ...
			 ((null (cddr args))
			  ...
			  (set (first args)
                               (old-eval-in-lexenv (second args) lexenv)))
		       (set (first args)
                            (old-eval-in-lexenv (second args) lexenv))))
		  (let ((symbol (first name)))
		    (case (info :variable :kind symbol)
		      (:special)
		      (t (return (%old-eval original-exp lexenv))))
		    (unless (type= (info :variable :type symbol)
				   *universal-type*)
		      ;; let the compiler deal with type checking
		      (return (%old-eval original-exp lexenv)))))))
	     ((progn)
	      (old-eval-progn-body (rest exp) lexenv))
	     ((eval-when)
                       ...
                       (let ((sb!c:*lexenv* lexenv))
                         (sb!c::funcall-in-macrolet-lexenv
                          ...
                          (lambda (&key funs)
                            ...
                            sb!c:*lexenv*)
                          :eval))))
                  (old-eval-locally `(locally ,@body) lexenv))))
	     ((symbol-macrolet)
                ...
                (multiple-value-bind (lexenv vars)
                    (let ((sb!c:*lexenv* lexenv))
                      (sb!c::funcall-in-symbol-macrolet-lexenv
                       ...
                       :eval))
                  (old-eval-locally `(locally ,@body) lexenv :vars vars))))
	     (t
	      (if (and (symbolp name)
		       (eq (info :function :kind name) :function))
		  (collect ((args))
                    (dolist (arg (rest exp))
                      (args (old-eval-in-lexenv arg lexenv)))
...
(defun eval-in-lexenv (exp lexenv)
  #!+sb-eval
  (if sb!eval:*use-old-eval*
      (old-eval-in-lexenv exp lexenv)
